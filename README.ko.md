...
```
You: /agent-harmony:run 코드 품질을 분석하는 SaaS를 만들고 싶어
...
# 빌드 시작
/agent-harmony:run 인증과 팀 협업이 있는 투두 앱
```
...
| 린트 | 에러 0개 | 프로젝트 린터 |
| 테스트 커버리지 | >= 60% | pytest --cov / jest --coverage |
| 최대 파일 줄 수 | <= 500줄 | wc -l |
| 최대 함수 줄 수 | <= 80줄 | 가장 큰 함수의 줄 수 |
...
```bash
# 세션 중단 또는 레이트 리밋
# /agent-harmony:run를 다시 실행하면 저장된 상태를 감지
/agent-harmony:run
...
| 명령어 | 설명 |
|--------|------|
| `/agent-harmony:run [설명]` | **한 줄이면 프로덕션까지.** 대화 → PRD → 셋업 → 빌드 → 완료 |
